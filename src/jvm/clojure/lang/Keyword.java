...
public final class Keyword implements IFn, Comparable, Named, Serializable {
    ...
    private static Map<Symbol, WeakReference<Keyword>> table =
        new WeakHashMap();
    ...
    public static Keyword intern(Symbol sym){
        try {
            lock.lock();
            WeakReference<Keyword> wexistingk = table.get(sym);
            if (wexistingk == null || wexistingk.get() == null) {
                wexistingk = new WeakReference(new Keyword(sym));
                table.put(sym,wexistingk);
            }
            return wexistingk.get();
        } finally {
            lock.unlock();
        }
    }
